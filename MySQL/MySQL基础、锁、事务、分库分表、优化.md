...
## 1. 数据库的三范式（最重要的三范式）是什么？

> 数据库其实有六个范式，但是前三个范式是最重要的！

- 第一范式：强调的是列的原子性，即数据库表的每一列都是不可分割的原子数据项。

  更具体地说就是每个列只能存储一个值，不能再继续拆
  下面这个例子就是不符合第一范式的：
...
- 第二范式：要求实体的属性完全依赖于主关键字。所谓完全 依赖是指不能存在仅依赖主关键字一部分的属性。（非主键字段必须完全依赖**整个主键**）
  这里的整个主键是重点，说明是联合主键
  现在有个表：

  | 学号 | 课程ID | 学生名字 | 课程名字 | 成绩 |
  | ---- | ------ | -------- | -------- | ---- |
  | 1    | 101    | 张三     | 数学     | 90   |
  | 1    | 102    | 张三     | 英语     | 85   |
  ...
  学生姓名 依赖于 学号；课程名字 依赖于 课程ID；成绩 依赖于 学号和课程ID，也就是整个主键（联合主键）；前两个就叫做**部分依赖**，因为只依赖了联合主键的部分。

  怎么修改呢？把学生姓名和课程名字这两个属性抽离出去，分别单独建立表
...
- 第三范式：任何非主属性不依赖于其它非主属性。
  ...
  依赖关系呢？员工ID -> 员工姓名 ； 部门ID -> 部门名称

  所以 员工ID和部门名称之间的关系是间接依赖。
