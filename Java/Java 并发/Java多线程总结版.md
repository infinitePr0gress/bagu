...
![ThreadLocal原理图](C:\Users\heban\Desktop\java简历and面试指北\Java-Interview-main\Java并发\assets\ThreadLocal原理图.png)
...
**创建一个==单线程化==的Executor**，即只创建唯一的工作者线程来执行任务，它只会用唯一的工作线程来执行任务，保证所有任务按照指定顺序(FIFO, LIFO, 优先级)执行。如果这个线程异常结束，会有另一个取代它，保证顺序执行。单工作线程最大的特点是可保证顺序地执行各个任务，并且在任意给定的时间不会有多个线程是活动的。
